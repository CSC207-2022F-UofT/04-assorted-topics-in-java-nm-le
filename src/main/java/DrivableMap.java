...
class DrivableMap {
     ...
     */
    public boolean addDrivable(String ID, Drivable animal) {
        if (drivable_map.containsKey(ID)) {
            return false;
        }
        this.drivable_map.put(ID, animal);
        return true;
    }
    ...
    public boolean hasFasterThan(int speed) {
        for (Drivable animal : this.drivable_map.values()) {
            if (animal.getMaxSpeed() >= speed) {
                return true;
            }
        }
        return false;
    ...
    public List<Tradable> getTradable() {
        ArrayList tradeable = new ArrayList<Tradable>();
        for (Drivable animal : this.drivable_map.values()) {
            if (animal instanceof Tradable) {
                tradeable.add(animal);
            }
        }
        return tradeable;
    }
